...
class VPythonFrontend(axtools.Frontend):
    def __init__(self):
        super().__init__()
    ...
    def run(self, args: axtools.AxiniteArgs):
        ...
        for body in args.bodies:
            ...
            spheres[body.name] = sphere(pos=self.to_vec(body.r[0]), radius=body.radius.value * args.radius_multiplier, color=body_color, make_trail=True, retain=body_retain, interval=10)
            ...
            if body.light == True: lights[body.name] = local_light(pos=self.to_vec(body.r[0]), color=body_color)

        
        def fn(t, **kwargs):
            rate(args.rate)
            for body in kwargs["bodies"]:
                spheres[body.name].pos = self.to_vec(body.r[t.value])
