...
def verlet_nojit_backend(delta, limit, bodies, action=None, modifier=None, t=0.0, action_frequency=200):
    ...
    for i, body in enumerate(bodies):
        ...
        if modifier is not None: f = modifier(body, f, bodies=ax.create_outer_bodies(bodies, limit, delta), t=t, delta=delta, limit=limit)
    ...
    while t < limit or _infinite:
        ...
        if action is not None and n % action_frequency == 0: action(ax.create_outer_bodies(bodies, limit, delta), t, limit=limit, delta=delta, n=n)
        n += 1
        t += delta
