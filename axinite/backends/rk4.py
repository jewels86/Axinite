...
def rk4_nojit_backend(delta, limit, bodies, action=None, modifier=None, t=0.0, action_frequency=200):
    ...
    while t < limit:
        ...
        for i, body in enumerate(bodies):
            ...
            body["v"][n] = body["v"][n-1] + (delta/6) * (a + 2 * (_bodies[i]["v_mid1"] + _bodies[i]["v_mid2"]) + _bodies[i]["v_mid3"])
            body["r"][n] = body["r"][n-1] + (delta/6) * (body["v"][n-1] + 2 * (_bodies[i]["v_mid1"] + _bodies[i]["v_mid2"]) + _bodies[i]["v_mid3"])
